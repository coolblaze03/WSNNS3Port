...
namespace ns3 {  
  ...
  NS_LOG_UNCOND ("NEW CALLBACK E");
		    if (sock == 0){
			NS_LOG_UNCOND ("Sink Sock Nothing");
		    }else{
			NS_LOG_UNCOND ("Sink Sock Something");
			m_socket = sock;
		    }*/

		  m_socket->SetRecvCallback (MakeCallback (&ns3::SensorApplication::HandleReceive, this));
		  NS_LOG_UNCOND (SensorNodePtr->NodeID <<  " Startring: " <<Simulator::Now ().GetSeconds());
...
counter++;
          ...
          void systemCheck(){
			...
			*/
            ...
            //NS_LOG_UNCOND (SensorNodePtr->NodeID <<  " Remaining Energy: "<< remainingenergy << " Time:" <<Simulator::Now ().GetSeconds());
          ...
          void SendToChildren (message m){
                ...
                for (unsigned int i = 0; i < SensorNode->childrenNodes.GetN(); i++){
                	...
                	NS_LOG_UNCOND (SensorNode->NodeID <<  " Sending Packet to: "<< cnode->NodeID);

                     Ptr<Ipv4> ipv4 = SensorNode->childrenNodes.Get(i)->GetObject<Ipv4>(); 
                     Ipv4InterfaceAddress iaddr = ipv4->GetAddress (1,0);
                        //NS_LOG_UNCOND (ciaddr.GetLocal()<<  " Sending Packet to: "<<iaddr.GetLocal());
          ...
          void SendToOther (message m){
                         ...
                         for (unsigned int i = 0; i < SensorNode->neighbors.GetN(); i++){
							 ...
							 NS_LOG_UNCOND (SensorNode->NodeID <<  " Sending Packet Parent: "<< cnode->NodeID);
      		  ...
      		  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
      		      ...
      		      if (DynamicCast<olsr::RoutingProtocol> (temp))
      			{
      			  ...
      			  for(iter = rte.begin(); iter != rte.end(); iter++){
      				...
      				if ((iter)->distance == 1){
      					NS_LOG_UNCOND (SensorNodePtr->NodeID <<  " Sending Packet to: "<< (iter)->destAddr.Get());
      						  ...
      						  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
      						      ...
      						      if (DynamicCast<olsr::RoutingProtocol> (temp))
      							{
      								  ...
      								  for(iter = CnSet.begin(); iter != CnSet.end(); iter++){
      										...
      										NS_LOG_UNCOND (SensorNodePtr->NodeID <<  " Sending Packet to: "<< (iter)->neighborMainAddr);
...
*/
	...
	void SendToSink(message m){
			...
			if (sinkAdd.Get() != 0){
				NS_LOG_UNCOND (SensorNodePtr->NodeID <<  " Sending Packet to Sink: "<< sinkAdd << " Time:"<<Simulator::Now ().GetSeconds());
			        m_socket->Connect (InetSocketAddress (sinkAdd,80));
			     	string mess_output = m.Output();
				Ptr<Packet> pkt1 = Create<Packet> (reinterpret_cast<const uint8_t*> (mess_output.c_str()), mess_output.length());
				m_socket->Send (pkt1);
			}
...
*/
	...
	Ipv4Address sinkAdd; 
		...
		NS_LOG_UNCOND ("Sensor MRX ID:" << SensorNodePtr->NodeID);
                Ptr<Packet> packet;
                  Address from;
