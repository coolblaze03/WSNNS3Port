...
class SinkApplication:public Application{
        ...
        private:
          ...
          virtual void StartApplication (void){
		    ...
		    if (m_socket == 0)
                    {
                      NS_LOG_UNCOND ("Sink Make Socket");
                  ...
                  NS_LOG_UNCOND (SinkNodePtr->NodeID <<  " Startring: " <<Simulator::Now ().GetSeconds());
   ...
   m_socket->SetRecvCallback (MakeCallback (&ns3::SinkApplication::HandleReceive, this));
                  ...
                  if (SinkNode->Active == true){
                	  ...
                	  NS_LOG_UNCOND ("Power On: "<<SinkNode->NodeID);
                	  powerOn();
                	  	/*message m;
						m.type = TREESETUP;
          ...
          void runCycle( double deltaTime)
			...
			void run(){
				...
				//NS_LOG_UNCOND ("Running Run Sink");
          ...
          void SendToChildren (message m){
                ...
                for (unsigned int i = 0; i < SinkNode->childrenNodes.GetN(); i++){
                	Ptr<WSNBaseNode> cnode = DynamicCast <WSNBaseNode> (SinkNode->childrenNodes.Get(i));
                     ...
                     NS_LOG_UNCOND (SinkNode->NodeID <<  " Sending Packet to: "<< cnode->NodeID);
		  ...
		  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
		      ...
		      if (DynamicCast<olsr::RoutingProtocol> (temp))
			{
			  ...
			  for(iter = rte.begin(); iter != rte.end(); iter++){
				...
				if ((iter)->distance == 1){
					NS_LOG_UNCOND (SinkNodePtr->NodeID <<  " Sending Packet to: "<< (iter)->destAddr.Get());
				  ...
				  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
				      ...
				      if (DynamicCast<olsr::RoutingProtocol> (temp))
					{
						  ...
						  for(iter = CnSet.begin(); iter != CnSet.end(); iter++){
								...
								NS_LOG_UNCOND (SinkNodePtr->NodeID <<  " Sending Packet to: "<< (iter)->neighborMainAddr);//.Get());
