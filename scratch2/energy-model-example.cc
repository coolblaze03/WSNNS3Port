      ...
      if (packet->GetSize () > 0)
        {
          ...
          NS_LOG_UNCOND ("--\nReceived one packet! Socket: "<< iaddr.GetIpv4 ()
           ...
           socket->Send (Create<Packet> (200));
            NS_LOG_UNCOND ("--\nSent one packet! Socket: "<< " Out REC."
                                                            << " port: " << "80" << " at time = " <<
                         Simulator::Now ().GetSeconds () << "\n--");
  ...
  if (pktCount > 0)
    {
      ...
      //InetSocketAddress iaddr = InetSocketAddress::ConvertFrom (from);
      NS_LOG_UNCOND ("--\nSent one packet! Socket: "<< iaddr.GetLocal ()
                                                            << " port: " << "80" << " at time = " <<
                         Simulator::Now ().GetSeconds () << "\n--");


      //NS_LOG_UNCOND ("--\nSent one packet! Socket: "<< "Test"
      //                                                      << " port: " << "80" << " at time = " <<
       //                  Simulator::Now ().GetSeconds () << "\n--");
    }
  else
...
main (int argc, char *argv[])
{
  ...
  basicSourceHelper.Set ("BasicEnergySourceInitialEnergyJ", DoubleValue (.5)); //energy source intial joules 
...
class WSNSensorNode : public Node
{
